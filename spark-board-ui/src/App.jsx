...
function applyTransformationNodeEffectOnSelectedTransformation(currentNode, selectedTransformationNode) {
    if (currentNode.id === selectedTransformationNode.id) {
        currentNode.style = { ...currentNode.style, fontWeight: "bold" };
    } else {
        currentNode.style = { ...currentNode.style, fontWeight: "normal" };
    }
    return currentNode;
}
...
export default function App() {
    ...
    const [selectedTransformation, setSelectedTransformation] = React.useState(model_initialNodes[0]);
    ...
    // Effect over transformation when selected transformation
    useEffect(() => setNodes((nds) => nds.map((node) => {
        if (node.type == "transformation") {
            return applyTransformationNodeEffectOnSelectedTransformation(node, selectedTransformation);
        }
        return node;
